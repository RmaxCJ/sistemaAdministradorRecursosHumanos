...
<head>
	...
	function validar()
	{
		...
		{	document.forma.action="subir_archivo_tabuladores.php?accion=guardar";
...
$success = 0;
?>
<p>Subir Archivo de Tabuladores</p>
		...
		{	
			...
			if($ext[1]=='xls' || $ext[1]=='xlsx')
			{
				...
				{	chmod($path.$archivo_destino, 0644 );
					...
					if($obj_archivo->sube_archivo($datos, 33))
					{	
						$leyenda = 'Todo Bien';
						$success = 1;
						$accion = 'envia';
...
if($accion == 'envia')
{
	...
	$archivo = $obj_archivo->max_archivo($anio, 33);
	...
	//$obj_archivo->borra_tabulador($anio);
	...
	for($i=$rowinit;$i<=$cols;$i++) 
	{
	    ...
	    $paso = 0;
		if($obj_archivo->registro_batch_tabulador($datos, $anio))
			$paso = 1;
